...
function validateItem<T>(value: T[keyof T], validation: ValidationAtom): string | undefined {
...
export type ValidationSchema<K extends object> = Record<keyof K, ValidationAtom>

function useValidation<T extends object>(values: T, schema: ValidationSchema<T>): [Record<keyof T, string> | undefined, () => void] {
  ...
  const validate = () => {
    const result =
      (Object.keys(values) as (keyof T)[])
        .reduce((acc, key) => {
          const v = values[key];
          const atom = schema[key];
          return { ...acc, [key]: validateItem<T>(v, atom) };
